...
#[derive(Debug)]
struct LocaleResource<'a> {
    path: &'a Path,
    entries: &'a [LocatedEntry],
}
...
fn read_locale_resources<'a>(cache: &'a CheckLocaleCache, locale: &str) -> Vec<LocaleResource<'a>> {
    cache
        .files(locale)
...
fn collect_definitions(resources: &[LocaleResource<'_>]) -> Definitions {
